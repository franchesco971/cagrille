...
class SyliusProductPersistence implements ProductPersistenceInterface
{
     ...
     *
     * Supprime d'abord les ProductImage de type "aliexpress" déjà attachées
     * (orphan-removal Doctrine les effacera en base), puis recrée une
     * ProductImage par URL, en stockant l'URL directement dans `path`.
     * La première image est marquée de type "main".
     */
    private function mapImages(ProductInterface $product, ProductDto $dto): void
    {
        ...
        // Supprime les images AliExpress existantes (orphan-removal = DELETE automatique)
        foreach ($product->getImages() as $existing) {
            if (in_array($existing->getType(), ['aliexpress', 'aliexpress_thumbnail'], true)) {
        ...
        foreach (array_values($dto->images) as $position => $url) {
            ...
            $image->setPath((string) $url);
            $image->setType($position === 0 ? 'main' : 'aliexpress_thumbnail');
