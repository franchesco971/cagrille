 ...
 *   POST https://openapi-api.alibaba.com/rest/auth/token/create  (signé HMAC)  → access_token
 */
#[AsCommand(
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($code === null) {
            ...
            $authUrl = self::AUTH_URL . '?' . http_build_query([
                ...
                'redirect_uri'  => 'http://localhost:14080/callback',
                'response_type' => 'code',
                'view'          => 'web',
                'sp'            => 'alibaba',
            ]);
            ...
            $io->writeln('  http://localhost:14080/callback?code=<CODE>');
            $io->newLine();
            $io->text('La page affichera une erreur 404, c\'est normal — copiez juste le paramètre "code" dans l\'URL.');
            $io->newLine();
        ...
        // IOP signing : HMAC-SHA256(key=appSecret, message=apiPath + sortedKeyValues)
        // cf. https://openapi.alibaba.com/doc/api.htm#/api?cid=4&path=/auth/token/create
        $signStr = $apiPath;
