        ...
        $rootNode
            ->children()
                ...
                ->scalarNode('access_token')
                    ->defaultValue('')
                    ->info('Token OAuth initial (optionnel — remplacé par var/aliexpress_token.json après le flux OAuth)')
                ->end()
                ->scalarNode('redirect_uri')
                    ->defaultValue('https://localhost/aliexpress/callback')
                    ->info('URI de callback OAuth (doit correspondre à celle enregistrée sur le portail AliExpress)')
                ->end()
                ->scalarNode('base_url')
